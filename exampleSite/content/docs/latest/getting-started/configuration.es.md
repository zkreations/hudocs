...
Las opciones específicas de documentación se configuran bajo `[params.docs]`.
 
* `section`: nombre de la sección raíz de documentación (por defecto `"docs"`).
* `default_version`: versión canónica activa (por defecto `"latest"`).
* `versions`: lista explícita y orden de las versiones para el selector. Si se omite, Hudocs detecta automáticamente las versiones desde `content/docs/`. Establecer en `false` para modo monoversión.
